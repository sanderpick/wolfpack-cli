/*
 * client.js: Client base for the Wolfpack API clients.
 *
 * (C) 2010, Wolfpack Inc.
 *
 */
...
// for communicating with Wolfpack's API
//
...
// code from Wolfpack matches `failCodes`.
//
Client.prototype.request = function (method, uri /* variable arguments */) {
  ...
  return this._request(options, function (err, response, body) {
    ...
    // if (!poweredBy || poweredBy.indexOf('Wolfpack') === -1) {
    //   error = new Error('Grr requires you to connect to Wolfpack\'s stack (api.grr.io)');
    ...
    if (failCodes[statusCode]) {
      error = new Error('Wolfpack Error (' + statusCode + '): ' + failCodes[statusCode]);
