...
common.checkVersion = function (callback) {
  ...
  //
  // Check the GitHub tags for `grr` to see if the current
  // version is outdated. If it is not make sure to message the user at the end.
  //
  request({
    uri: 'http://registry.npmjs.org/grr/latest',
    timeout: 400
    ...
    if (!responded) {
      ...
      try {
        ...
        if (semver.gt(pkg.version, grr.version)) {
          grr.log.warn('A newer version of ' + 'grr'.magenta + ' is available. ' + 'please update immediately');
          grr.log.help('To install the latest ' + 'grr'.magenta + ' type `[sudo] npm install grr -g`');
          grr.log.warn('To ' + 'continue'.bold + ' without an update ' + 'type'.cyan  + ' ' + '\'yes\''.magenta);

          grr.prompt.confirm('Continue without updating? Bad things might happen (no)', function (err, result) {
