...
teams.usage = [
  ...
  'is supplied, grr will attempt to read the package.json',
...
function handleStartError(err, name, callback) {
  ...
  if (err.result) {
    ...
    if (err.result.error === 'not_found') {
      jitsu.log.error('App ' + name.magenta + ' doesn\'t exist on Wolfpack yet!');
      return callback({});
    }
  }
  callback(err);
...
teams.add = function (add, name, callback) {
  ...
  grr.teams.update([username, name].join('/'),
                  {$addToSet: {users: user}}, callback);
...
// //
// teams.list.usage = [
//   'Lists all of the teams for the current user',
//   '',
//   'grr list',
//   'grr teams list'
// ];
